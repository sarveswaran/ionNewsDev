...
@section('content-header')
    <h1>
        {{ trans('crawl::crawlcontents.title.edit crawlcontent') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.crawl.crawlcontent.index') }}">{{ trans('crawl::crawlcontents.title.crawlcontents') }}</a></li>
        <li class="active">{{ trans('crawl::crawlcontents.title.edit crawlcontent') }}</li>
    </ol>
@stop
...
@section('content')
    {!! Form::open(['route' => ['admin.crawl.crawlcontent.update', $crawlcontent->id], 'method' => 'put']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('crawl::admin.crawlcontents.partials.edit-fields', ['lang' => $locale])
                        </div>
                    @endforeach
